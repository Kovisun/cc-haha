  ...
  switch (cliMsg.type) {
    case 'assistant': {
      ...
      if (cliMsg.message?.content && Array.isArray(cliMsg.message.content)) {
        ...
        // NOT reset here: multiple assistant messages may arrive per turn
        // (--include-partial-messages) and each should skip text/thinking
        // that was already sent via stream_events. Reset happens in the
        // result handler (end of turn).
        return messages
      }
      return []
    }
    ...
    case 'result': {
      ...
      // Reset stream state for the next turn. The assistant handler no longer
      // resets hasReceivedStreamEvents so multiple partial assistant messages
      // don't re-emit text/thinking that was already sent via stream_events.
