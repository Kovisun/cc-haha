...
async function handleWechatAdaptersApi(req: Request, tail: string[]): Promise<Response> {
  const WECHAT_DEFAULT_BASE_URL = 'http://localhost:8089'
  const protocol = await import('../../../adapters/wechat/protocol.js').catch(() => null)
  if (!protocol) {
    throw ApiError.notFound('WeChat 适配器未安装')
  }

  if (req.method === 'POST' && tail[0] === 'login' && tail[1] === 'start') {
    const result = await protocol.startWechatLoginWithQr({ force: true })
  ...
  if (req.method === 'POST' && tail[0] === 'login' && tail[1] === 'poll') {
    ...
    const result = await protocol.pollWechatLoginWithQr({ sessionKey: body.sessionKey })
    if (result.connected) {
      await adapterService.updateConfig({
        wechat: {
