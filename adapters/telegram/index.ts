...
const TELEGRAM_TEXT_LIMIT = 4090 // leave margin below 4096
...
let botOptions: Record<string, any> = {}
if (apiRoot) botOptions.client = { apiRoot }
if (proxyUrl) {
  const { SocksProxyAgent: SPA } = await import("socks-proxy-agent")
  botOptions.client = { ...(botOptions.client || {}), baseFetchConfig: { agent: new SPA(proxyUrl) } }
}
const bot = Object.keys(botOptions).length > 0
  ? new Bot(config.telegram.botToken, botOptions)
...
// Typing keepalive timers — sends sendChatAction every 5s while thinking/executing
  ...
  if (placeholder) {
    if (isComplete) {
      ...
      try {
        ...
        // editMessageText failed — fall back to sendMessage for all chunks
...
async function handleServerMessage(chatId: string, msg: ServerMessage): Promise<void> {
  ...
  // DEBUG: log every message type and text snippet
  if (msg.type !== 'thinking') {
    const snippet = msg.text ? (msg.text.slice(0, 80).replace(/\n/g, '\\n')) : (typeof msg.message === 'string' ? msg.message.slice(0, 80) : '')
    console.log(`[Telegram:msg] type=${msg.type}${msg.blockType ? ` blockType=${msg.blockType}` : ''}${msg.state ? ` state=${msg.state}` : ''}${snippet ? ` text="${snippet}"` : ''}`)
  }
  ...
  switch (msg.type) {
        ...
        if (placeholders.has(chatId)) {
          ...
          if (text?.trim()) {
            ...
            try {
              ...
              // If edit fails (e.g. stale message), send as new message instead
              for (const chunk of chunks) {
                await bot.api.sendMessage(numericChatId, chunk)
              }
              placeholders.delete(chatId)
    ...
    case 'content_delta':
      if (msg.text) {
        // Create placeholder if content_start: text was never sent (e.g. some CLI
        // versions skip content_block_start for text blocks and rely on the
        // partial assistant message, which gets skipped when hasReceivedStreamEvents=true)
        if (!placeholders.has(chatId)) {
          console.log('[Telegram:msg] content_start: text (auto-created from content_delta)')
          stopTypingKeepalive(chatId)
          const sent = await bot.api.sendMessage(numericChatId, '▍')
          placeholders.set(chatId, { chatId, messageId: sent.message_id })
          accumulatedText.set(chatId, '')
    ...
    case 'message_complete':
      ...
      if (placeholders.has(chatId)) {
        ...
        accumulatedThinkingText.delete(chatId)
        buffers.get(chatId)?.reset()
      } else if (accumulatedText.has(chatId)) {
        // Orphaned text: content_start was never sent, flush remaining text directly
        const text = accumulatedText.get(chatId)
        accumulatedText.delete(chatId)
        if (text?.trim()) {
          const chunks = splitMessage(formatTelegramOutboundText(text), TELEGRAM_TEXT_LIMIT)
