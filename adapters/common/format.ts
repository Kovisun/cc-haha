...
/** Split text into chunks that fit within a character limit, respecting paragraph/sentence boundaries and table integrity. */
export function splitMessage(text: string, limit: number): string[] {
  ...
  while (remaining.length > 0) {
    ...
    // Table-aware: if split falls inside a pipe table block, back up to before the table
    if (remaining[splitAt] === '\n') {
      const afterLine = remaining.slice(splitAt + 1).split('\n')[0]?.trim() ?? ''
      const beforePart = remaining.slice(0, splitAt)
      const beforeNewline = beforePart.lastIndexOf('\n')
      const currentLine = (beforeNewline >= 0 ? beforePart.slice(beforeNewline + 1) : beforePart).trim()
      if (currentLine.startsWith('|') && afterLine.startsWith('|')) {
        const lines = beforePart.split('\n')
        let tableStart = lines.length - 1
        while (tableStart >= 0 && lines[tableStart]!.trim().startsWith('|')) tableStart--
        tableStart++
        const adjusted = lines.slice(0, tableStart).join('\n').length
        if (adjusted > 0 && adjusted <= limit) splitAt = adjusted
      }
    }
