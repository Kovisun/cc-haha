...
export class WsBridge {
  ...
  private connect(chatId: string, sessionId: string): void {
    ...
    // Cancel any pending reconnect timer and properly close old WS
    const prev = this.sessions.get(chatId)
    if (prev) {
      if (prev.reconnectTimer) clearTimeout(prev.reconnectTimer)
      // Close the old WS so the server-side cleanup (removeActiveClient)
      // runs before the new connection registers. removeAllListeners()
      // alone leaves the old TCP connection open on the server side,
      // causing duplicate output callbacks for the same session.
      prev.ws.removeAllListeners()
      if (prev.ws.readyState === WebSocket.OPEN || prev.ws.readyState === WebSocket.CONNECTING) {
        prev.ws.close(1000, 'reconnecting')
      }
    }
    ...
    ws.on('error', (err) => {
      console.error(`[WsBridge] Error on ${sessionId}:`, err?.message ?? err)
    })
  }
