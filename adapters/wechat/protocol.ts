...
export async function pollWechatLoginWithQr(opts: {
  ...
  switch (status) {
    ...
    case 'scaned_but_redirect':
      if (rawStatus.redirect_host) {
        login.currentApiBaseUrl = `https://${rawStatus.redirect_host}`
      }
    ...
    case 'confirmed':
      activeLogins.delete(opts.sessionKey)
      if (!rawStatus.bot_token || !rawStatus.ilink_bot_id) {
      ...
      return {
        ...
        status: 'confirmed',
        message: '微信绑定成功。',
        botToken: rawStatus.bot_token,
        accountId: rawStatus.ilink_bot_id,
        baseUrl: rawStatus.baseurl || login.currentApiBaseUrl || WECHAT_DEFAULT_BASE_URL,
        userId: rawStatus.ilink_user_id,
      }
    default:
      console.warn('[WeChatProtocol] unknown QR status:', rawStatus)
      return { connected: false, status: 'wait', message: '等待微信扫码确认。' }
  }
}
